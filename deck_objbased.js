...
var player1Bank = new Bank(5000.00)
var dealer = new Player('dealer')
var dealerBank = new Bank(100000000.00)
...
function doBet() {
    var brang = $('#betRange')
    $(brang).attr('min',10).attr('max', 500 > player1Bank.balance ? player1Bank.balance : 500.00).val(10)
    $('#betModal').modal('show')
...
function whatNext(_player) {
    ...
    $('#player-buttons').empty()
     if (_player.canHit()) {
         ...
         $('#player-buttons').append(hitBtn).fadeIn(200)
     }

     if (_player.canSplit()) {
         var splitBtn = createButton('split', 'Split', _player)
         $('#player-buttons').append(splitBtn).fadeIn(200)
     }

     var stayBtn = createButton('stay', 'Stay', _player)
     $('#player-buttons').append(stayBtn).fadeIn(200)

     if (_player.canBet()) {
         var betBtn = createButton('bet', 'Bet', _player)
         $('#player-buttons').append(betBtn).fadeIn(200)
     }
...
function dealersTurn(_dealer) {
    ...
    if (scoresofar >= 17 && scoresofar < 21) {
        dealerPlaying = false
...
///background-size: contain;
    $(div).addClass('panel').addClass('m-2')
    if (facing === 'up') {
        ...
        $(div).css('background-image', 'url(' + card.image + ')').css('min-width', '160px').css('min-height', '223px').css('background-size','contain').fadeIn(500)
    }
    if (facing === 'down') {
        $(div).attr('data-side', 'back').attr('data-suit', card.suit).attr('data-value',
            card.value).attr('data-cardback', '/cardback.png').attr('data-cardfront', card.image)
        $(div).css('background-image', 'url("/cardback.png")').css('min-width', '160px').css('min-height', '223px').css('background-size', 'contain').fadeIn(500)
    }
    if (who.type === 'player') {
        $(div).click(function (event) {
            ...
            if (side === 'back') {
                ...
                $(thecard).css('background-image', `url(${imagefront})`).fadeIn(500)
            } else {
                $(thecard).attr('data-side', 'back')
                var imageback = $(thecard).attr('data-cardback')
                $(thecard).css('background-image', `url(${imageback})`).fadeIn(500)
            }
        })
    ...
    if (who.isSplit) {
        if (who.splittingTop) {
             $('#' + who.type + '-cards-split-top').append(div).fadeIn(600)
        }
        if (who.splittingBottom) {
             $('#' + who.type + '-cards-split-bottom').append(div).fadeIn(500)
        }
    } else {
         $('#' + who.type + '-cards').append(div).fadeIn(500)
    }
    if (who.type === 'player') {
...
function flipcard(card) {
    ...
    $(c).css('background-image', `url(${img})`).fadeIn()
      displayScore(dealer)
}
function doWin() {
...
function createAlert(message) {
    var div = $('<div>')
    $(div).addClass('alert').addClass('alert-primary').addClass('fade').addClass('show').addClass('col-12').attr('role', 'alert')
    var mess = $('<strong>')
    $(mess).text(message)
    $(div).append(mess)
    $(div).alert();
...
function createButton(btnName, btnTitle, player) {
    ...
    $(btn).attr('data-player', player.type).addClass("mr-3").addClass("ml-3").addClass("btn").addClass("btn-light")
    $(btn).text(btnTitle).click(function (event) {
...
function countFace(cards = []) {
    ...
    if (typeof (cards) === 'object') {
         if (cards.value === 'KING' || cards.value === 'QUEEN' || cards.value === 'JACK') {
             count++
         }
...
function displayScore(player) {
    ...
    $('#' + player.type + '-points').text(pp).fadeIn()
    return pp
}
/** Event Handlers */
